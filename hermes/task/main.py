...
class task():
    ...
    @failsafe
    def __init__(self, settings = None, name = None, user = 'default', logless = False, allquiet = False) -> None:
        ...
        # setup logger (if not logless mode) & this task name
        if logless is False:
            try:   self.log = tasklogger(path = TASKP, name = name)
            except Exception as err:
                raise Exception('error in logger setup') from err
                
            self.name = self.log.name  # name is eventually fixed by the logger
        else:
            print('[task] logless mode')
            self.log = None  # run in logless mode (send messages in bot)
            if name is None: self.name = 'logless'
        ...
        try: self.__notify( mstd.init.format(self.name) )
        except Exception as err:
            raise Exception('internal notify error (creation msg)') from err
        
        print("[task] <{}> has been initialized".format(self.name) )
    ...
    # for public use (i.e. with failsafe & task name included in message)
    @failsafe
    def notify(self, txt = '', img = None, quiet = False) -> None:
        ...
        self.__notify(txt = "[{}] ".format(self.name) + txt, imgb = img, quiet = quiet)
    ...
    def plotify(self, module : ModuleType,
        ...
        buf.seek(0)
        self.__notify(txt = "[{}] ".format(self.name) + txt, imgb = buf, quiet = quiet)
        
        # ? keep the picture
        buf.seek(0)
        if isinstance(keep, bool):
            if keep is True:
                fname = self.name + datetime.now().strftime(PICTURE_TIME_FORMAT) + PICTURE_DEFAULT_EXT
    ...
    def waypoint(self, txt = None, timed = False, reset = False) -> None:
        ...
        msg = mstd.waypoint.format(self.name, self.wpoint_count)
        if txt is not None:
            msg += ' ' + txt
        if timed:
    ...
    def close(self, fail = None):
        ...
        if fail is None:
            log = 'closed'
            msg = mstd.closed.format(self.name)
        elif isinstance(fail, str):
            log = 'failed, err message will follow:\n\n' + fail
            msg = mstd.failed_msg.format(self.name, fail)          
        else:
            log = 'failed'
            msg = mstd.failed.format(self.name, fail)
